...
public class Restablecercontrasea extends VistaRestablecercontrasea {
	...
	private void configurarVistaInicial() {
		...
		this.getMessageBox().add(new com.vaadin.flow.component.html.Paragraph(
	...
	private void verificarCorreoYContinuar(String correo) {
		try {
			...
			if (usuario != null) {
				...
				// No cambiar el estado solicitandoCorreo para que se pueda cancelar
				// correctamente
	...
	private void validarNuevaContrasena() {
		...
		if (!nuevaContrasena.matches(regex)) {
			mostrarMensajeError(
					"La contraseña debe tener entre 8-14 caracteres, al menos una mayúscula y un carácter especial");
			return;
		}
