...
public class Verhashtagregistrado extends Verhashtag {
	...
	private Tweet[] crearTweetsMockup() {
		...
		try {
			...
			// Tweet 6 (exclusivo para registrados)
			tweets[5] = new Tweet();
			tweets[5].setContenidoTweet("Evento exclusivo sobre " + _hashtag.getHashtag() + " la próxima semana 🎉 ¡Regístrate ya! #evento");
			tweets[5].setFechaPublicacion(new java.util.Date(System.currentTimeMillis() - 28800000)); // 8 horas atrás
	...
	private basededatos.Usuario_Registrado[] crearUsuariosMockup() {
		basededatos.Usuario_Registrado[] usuarios = new basededatos.Usuario_Registrado[4];
		...
		try {
			...
			// Usuario 4 (exclusivo para registrados)
			usuarios[3] = new basededatos.Usuario_Registrado();
			usuarios[3].setNickname("event_organizer");
			usuarios[3].setDescripcion("Organizador de eventos tecnológicos");
